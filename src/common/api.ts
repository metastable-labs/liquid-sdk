...
import { CreatePassKeyCredentialOptions, PasskeyAuthResult, PasskeyRegistrationResult } from '../types';
...
export class LiquidAPI {
    ...
    async verifyRegistration(username: string, registrationResponse: PasskeyRegistrationResult): Promise<{ verified: boolean; publicKey: string }> {
        return this.fetchWithErrorHandling(
            `${this.apiBaseUrl}/registration/verify`,
            {
                method: 'POST',
                headers: {
    ...
    async verifyAuthentication(username: string, authenticationResponse: PasskeyAuthResult): Promise<{ success: boolean }> {
        return this.fetchWithErrorHandling(
            `${this.apiBaseUrl}/authentication/verify`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
